    ...
    <div class="dashboard-main-body">
        <div class="d-flex flex-wrap align-items-center justify-content-between gap-3 mb-24">
            ...
            <ul class="d-flex align-items-center gap-2">
                <li class="fw-medium">
                    <a href="{{ route('product.edit',$product->id) }}" class="d-flex align-items-center gap-1 hover-text-primary">
                    ...
                    <form id="productForm" action="{{ route('product.update',$product->id) }}" enctype="multipart/form-data">
                            ...
                            <div class="card-body">
                                ...
                                <div class="row">
                                    ...
                                    <div class="col-12 mb-3">
                                        ...
                                        <textarea class="form-control" id="description" name="description" placeholder="A few words ..." rows="3">{{ ($product->description)? $product->description : '' }}</textarea>
                                    </div>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="card">
                            <div class="card-body">
                                ...
                                <button type="submit" id="btnSave" class="btn btn-primary" data-id="{{ $product->id }}">update</button>
...
<script>
    $(document).ready(function() {
        ...
        const selectedUnitId = "{{ $product->unit_id }}";
        ...
        // Initialize all Select2 dropdowns with consistent configuration
        ...
        // Load categories with optional selected ID
        function loadCategories(selectedId = null) {
            $.ajax({
                ...
                success: function(response) {
                    let $select = $('#category');
                    $select.empty().append('<option disabled>Choose Category</option>');
        ...
        // Load brands with optional selected ID
        function loadBrands(selectedId = null) {
            $.ajax({
                ...
                success: function(response) {
                    let $select = $('#brand');
                    $select.empty().append('<option disabled>Choose Brand</option>');
        ...
        // Load units with optional selected ID
        function loadUnits(selectedId = null) {
            $.ajax({
                ...
                success: function(response) {
                    let $select = $('#unit');
                    $select.empty().append('<option disabled>Choose Unit</option>');
        ...
        // Handle inline additions for all types (category, brand, unit)
        ...
        // Update add-button text in real time as user types
                    ...
                    if (xhr.status === 422) {
                        ...
                        $.each(errors, function(key, value) {
                            ...
                            if (key === 'bar_code') {
                                ...
                                $('.barcode_group').after(`<span class="text-danger validation-error">${value[0]}</span>`);
