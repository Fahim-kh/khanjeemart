...
<body>
    ...
    <div class="customer-info">
        <table width="100%">
            @if ($supplier != null)
                <tr>
                    <td width="50%"><strong>Supplier Name:</strong> {{ $supplier->name ?? 'N/A' }}</td>
                    <td width="50%"><strong>Phone:</strong> {{ $supplier->phone ?? 'N/A' }}</td>
                </tr>
                <tr>
            ...
            @if (count($ledgerData) > 0)
                @foreach ($ledgerData as $row)
                    ...
                    <tr>
                        ...
                        <td>
                            @if ($supplier != null)
                                @if (isset($row['purchase_id']) && $row['purchase_id'])
                                    {{ $row['reference'] ?? '---' }}
                                @else
                                    {{ $row['payment_type'] ?? '---' }}
                                @endif
